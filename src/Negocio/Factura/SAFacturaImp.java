...
public class SAFacturaImp implements SAFactura {
	...
	public TCarrito read(int codigo) {
		...
		return DAOAbstractFactory.getInstance().createDAOFactura().read(codigo);
		
	}

	public TFactura readByCodigo(int codigo) {
		return DAOAbstractFactory.getInstance().createDAOFactura().readByCodigo(codigo);
	}
	
	@Override
	public List<TFactura> readAll() {
		return DAOAbstractFactory.getInstance().createDAOFactura().readAll();
	}
	...
	public TFacturaConProductos readFacturaConProductos(int codigo) {	
		...
		aux.settCliente(DAOAbstractFactory.getInstance().createDAOCliente().readById(carrito.gettFactura().getCodigoCliente()));
	...
	public int cerrarFactura(TCarrito carrito) {
		...
		if(carrito != null && carrito.getMapa().size() > 0){
			...
			TCliente cliente = DAOAbstractFactory.getInstance().createDAOCliente().readById(carrito.gettFactura().getCodigoCliente());
			...
			if(cliente != null && cliente.isEstado() && trabajador != null && trabajador.isEstado()){ // si existen y estan activos 
				...
				Controller.getInstance().action("El cliente o el trabajador estan inactivos", Evento.DATOS_INCORRECTOS_FACTURA);
			}
	...
	public List<TContiene> readContieneDeFactura(int codigo){
		...
		return DAOAbstractFactory.getInstance().createDAOContiene().readById(codigo);
