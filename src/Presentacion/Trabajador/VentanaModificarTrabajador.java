		...
		botonAceptar.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(!DNI_Text.getText().equals("") && !NOMBRE_Text.getText().equals("")) {
					TTrabajador aux = new TTrabajador(DNI_Text.getText(),NOMBRE_Text.getText(),TELEFONO_Text.getText());
					Controller.getInstance().action(aux, Evento.MODIFICAR_TRABAJADOR);
				}else {
					Controller.getInstance().action("No has introducido datos", Evento.DATOS_INCORRECTOS_TRABAJADOR);
				}
				removeBox();
				setVisible(false);		
