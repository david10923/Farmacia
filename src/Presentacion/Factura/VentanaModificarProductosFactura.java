...
public class VentanaModificarProductosFactura extends JDialog {
	...
	private List<TContiene> contienes;
				...
				if(!id_producto_text.getText().equals("") && ((int)cantidad_producto.getValue()) != 0 ){
					...
					if(cont != null){
						...
						if(cont.getCantidad() >= (int)cantidad_producto.getValue()){	
							...
							Controller.getInstance().action("Has eliminado mas unidades de las que tienes", Evento.DATOS_INCORRECTOS_FACTURA);
						}
					}			
					...
					update(contienes);
					//contienes.remove(cont);
					//cont.setCantidad(unidadesQuitadas);
					//contienes.add(cont);
					removeBox();
				}else{
					Controller.getInstance().action("Los datos introducidos no son correctos", Evento.DATOS_INCORRECTOS_FACTURA);
				}
