...
public class MostrarUno extends JPanel{
		...
		combo.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (combo.getItemCount() > 0) {
					...
					switch (nameId) {
					case "Cliente":
						Controller.getInstance().action(id, Evento.MOSTRAR_CLIENTE);
						break;
					case "Trabajador":
	...
	public void llenarCombo() {
		...
		case "Cliente":
			...
			for(Object tCli : SAAbstractFactory.getInstance().createSACliente().readAll()) {
				combo.addItem(((TCliente) tCli).getCodigo() + " - " + ((TCliente) tCli).getDni());
			}
			break;
	...
	public void setInfo(Object aux) {
		...
		case "Cliente":
			info.setText(((TCliente)aux).toString());
			break;
		case "Trabajador":
